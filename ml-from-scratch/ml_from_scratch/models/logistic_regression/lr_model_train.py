...
from ml_from_scratch.utils.math_functions import sigmoid
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--early_stopping", default=False, action="store_true", help="Early Stopping"
    )
    parser.add_argument(
        "--num_iterations",
        type=int,
...
class Trainer:
    ...
    def cost_function(
        ...
        :param b: Bias
        :type b: Float
        :return: Cost function (value of the cost function computed on
            the whole training data)
        """
        assert (
            X.shape[1] == Y.shape[1]
        ), "X and Y should have the same number of training examples (i.e., columns)."
    ...
    @staticmethod
    def gradient(
        X: np.ndarray, Y: np.ndarray, Yhat: np.ndarray
    ) -> Tuple[np.ndarray, float]:
        num_samples = X.shape[1]
    ...
    @staticmethod
    def process(
        weights: np.ndarray, bias: float, data: np.ndarray, s: float = 0.5
    ) -> np.ndarray:
        """Compute prediction using model weights and bias.

        Parameters:
            - weights: Optimal value for W  (that minimize the cost function).
                    Vector of size (number_pixels * number_pixels * 3, 1)
            - bias (float): Optimal value for the bias (that minimize the cost function)
            - data: Test data matrix (to use for prediction)
    ...
    def model_assessment(
        ...
        """
        prediction_train_y = self.process(weights=w, bias=b, data=x_train)
        prediction_test_y = self.process(weights=w, bias=b, data=x_test)

        if verbose:
            logger.info(
                f"Train accuracy: {(100 - np.mean(np.abs(prediction_train_y - y_train)) * 100)} %"
            )
            logger.info(
                f"Test accuracy: {(100 - np.mean(np.abs(prediction_test_y - y_test)) * 100)} %"
            )
    ...
    def train(
        ...
        **kwargs,
        ...
        res = LRModel(weights=w_optimal, bias=b_optimal, costs=costs, classes=classes)
