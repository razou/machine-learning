...
from ml_from_scratch.models.neural_network.model_loader import NeuralNetModel
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--model_dir", type=str, default="neural_net_model", help="Model dir"
    )
    parser.add_argument(
        "--model_name", type=str, default="neural_net", help="Model name"
    )
    parser.add_argument(
...
def main():
    ...
    neural_net_model = NeuralNetModel.load_model(model_path=model_artefact_path)

    images_dir = os.path.join(ROOT_DIR, "data/images")
    for img in os.listdir(images_dir):
        img_path = os.path.join(images_dir, img)
        try:
            neural_net_model.predict_new_image(image_path=img_path, s=proba_threshold)
